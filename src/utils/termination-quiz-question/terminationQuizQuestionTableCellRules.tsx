...
interface CreateTerminationQuizQuestionTableCustomRendererParams {
...
export function createTerminationQuizQuestionTableCustomRenderer({
  questionColumnIndex,
  statusColumnIndex,
  onViewDetail,
  getStatusEnabled,
}: CreateTerminationQuizQuestionTableCustomRendererParams): TableCellCustomRenderer {
  return ({ row, value, columnIndex }) => {
    const tableRow: TableRow = row

    if (columnIndex == questionColumnIndex) {
      return (
        <button
          type="button"
