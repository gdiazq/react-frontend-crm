...
import { AUTH_ROUTE_SETTLEMENTS_SAFETY_COMPLIANCE_CREATE } from '@/constant'
...
interface SafetyComplianceListToolbarComponentProps {
  onOpenFilters: () => void
  disabled?: boolean
}

export function SafetyComplianceListToolbarComponent(props: SafetyComplianceListToolbarComponentProps) {
  const { onOpenFilters, disabled = false } = props
  const navigate = useNavigate()
  const search = useStoreSafetyCompliance((s) => s.queryParams.search)
  ...
  return (
    <>
      <form
        ...
        onSubmit={(event) => { event.preventDefault(); void searchSafetyCompliance() }}
      >
