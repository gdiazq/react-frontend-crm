...
export function ProjectTypeDetailComponent({
  ...
  return (
    <DetailStateWrapperComponent
      ...
      onRetry={onRetry}
    >
      {detail && <ProjectTypeDetailContent detail={detail} onEdit={onEdit} />}
    </DetailStateWrapperComponent>
  )
}
...
function ProjectTypeDetailContent({
  ...
  const statusLabel = detail.active ? 'Activo' : 'Inactivo'
  const statusTone = detail.active ? 'ok' : 'bad'
  const description = (
    <>
      Catálogo base para clasificar proyectos dentro del módulo de <span className="num">Proyectos</span>.
    </>
  )
