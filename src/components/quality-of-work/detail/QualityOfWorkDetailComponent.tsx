...
import type { QualityOfWorkDetailView } from '@/types'

interface QualityOfWorkDetailComponentProps {
  detail: QualityOfWorkDetailView | null
  loading: boolean
...
export function QualityOfWorkDetailComponent({
  detail,
  loading,
  errorMessage,
  onRetry,
  onEdit,
}: QualityOfWorkDetailComponentProps) {
  return (
    <DetailStateWrapperComponent
      loading={loading}
      errorMessage={errorMessage}
      hasData={detail !== null}
      loadingText="Cargando detalle de calidad del trabajo..."
      emptyText="Selecciona un registro para ver su detalle."
      onRetry={onRetry}
    >
      {detail && <QualityOfWorkDetailContent detail={detail} onEdit={onEdit} />}
    </DetailStateWrapperComponent>
  )
}

function QualityOfWorkDetailContent({
  detail,
  onEdit,
}: {
  ...
  const description = (
    <>
      Registro de evaluación para flujos de <span className="num">RRHH</span> y finiquito.
    </>
  )
