...
import { AUTH_ROUTE_SETTLEMENTS_WORK_QUALITY_CREATE } from '@/constant'
...
interface QualityOfWorkListToolbarComponentProps {
  onOpenFilters: () => void
  disabled?: boolean
}

export function QualityOfWorkListToolbarComponent(props: QualityOfWorkListToolbarComponentProps) {
  const { onOpenFilters, disabled = false } = props
  const navigate = useNavigate()
  const search = useStoreQualityOfWork((s) => s.queryParams.search)
  ...
  return (
    <>
      <form
        ...
        onSubmit={(event) => { event.preventDefault(); void searchQualityOfWork() }}
      >
