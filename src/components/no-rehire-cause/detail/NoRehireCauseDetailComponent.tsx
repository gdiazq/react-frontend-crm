...
export function NoRehireCauseDetailComponent({
  ...
  return (
    <DetailStateWrapperComponent
      ...
      hasData={detail !== null}
      loadingText="Cargando detalle de causa de no recontratación..."
      emptyText="Selecciona un registro para ver su detalle."
      onRetry={onRetry}
    >
      {detail && <NoRehireCauseDetailContent detail={detail} onEdit={onEdit} />}
    </DetailStateWrapperComponent>
  )
}
...
function NoRehireCauseDetailContent({
  ...
  const description = (
    <>
      Causa usada para flujos de <span className="num">RRHH</span> cuando el trabajador no es recontratable.
    </>
  )
