...
export function ProjectStatusDetailComponent({
  detail,
  loading,
  errorMessage,
  onRetry,
  ...
  return (
    <DetailStateWrapperComponent
      ...
      onRetry={onRetry}
    >
      {detail && <ProjectStatusDetailContent detail={detail} onEdit={onEdit} />}
    </DetailStateWrapperComponent>
  )
}
...
function ProjectStatusDetailContent({
  ...
  const description = (
    <>
      Catálogo base para definir vigencias dentro del módulo de <span className="num">Proyectos</span>.
    </>
  )
